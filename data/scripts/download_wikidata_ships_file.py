...
class DownloadWikidataShipsFile:
    WHERE_BASE = """
    ...
    def build_query(self, query_parts):
        return f"""
            SELECT
                ...
                {query_parts['select']}
            WHERE {{
                {DownloadWikidataShipsFile.WHERE_BASE}
    ...
    def get_queries(self):
        ...
        queries.insert(0, f"SELECT ?item ?imo WHERE {{ {DownloadWikidataShipsFile.WHERE_BASE} }}")
        return queries
...
if __name__ == "__main__":
    DownloadWikidataShipsFile("/tmp/wikidata.ships.csv").run()
