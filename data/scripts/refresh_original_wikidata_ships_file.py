...
# python3 scripts/refresh_original_wikidata_ships_file.py

class RefreshOriginalWikidataShipsFile:
    WHERE_BASE = """
    ...
    def build_query(self, query_parts):
        return f"""
            SELECT
                ...
                {query_parts['select']}
            WHERE {{
                {RefreshOriginalWikidataShipsFile.WHERE_BASE}
    ...
    def get_queries(self):
        ...
        queries.insert(0, f"SELECT ?item ?imo WHERE {{ {RefreshOriginalWikidataShipsFile.WHERE_BASE} }}")
        return queries
    ...
    def run(self):
        ...
        print(f"wrote output to {self.output_path}")


if __name__ == "__main__":
    RefreshOriginalWikidataShipsFile("/tmp/wikidata.ships.csv").run()
