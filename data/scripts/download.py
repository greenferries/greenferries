...
import sys
import datetime
from download_wikidata_ships_file import DownloadWikidataShipsFile
from download_wikidata_urls_file import DownloadWikidataUrlsFile

# ex python3 scripts/download.py thetis_2019
# ex python3 scripts/download.py wikidata_ships
...
class Download(object):
    ...
    def run(self):
        ...
        if match:
            self.download_thetis_file(match.groups()[0])
        elif self.data_source == "wikidata_ships":
            self.download_wikidata_ships_files()
        elif self.data_source == "wikidata_urls":
            self.download_wikidata_urls_files()
        self.write_metadata()
        print("done!")
